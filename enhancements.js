...
(function(){
  ...
  function exportData(){
    const payload={app:'زاد الذاكرين',version:window.ZAD_APP?.version||'1.0.0',exportedAt:new Date().toISOString(),data:appData()};
  ...
  async function importData(input){
    ...
    try{const payload=JSON.parse(await file.text());if(payload.app!=='زاد الذاكرين'||!payload.data||typeof payload.data!=='object')throw new Error('هذا ليس ملف نسخة احتياطية صالحًا');
      ...
      if(!confirm(`سيتم استعادة ${entries.length} عنصرًا مع الاحتفاظ ببيانات زاد الذاكرين غير الموجودة في الملف. هل تتابع؟`))return;
  ...
  function checkPrayerAlert(){
    ...
    (typeof PrayerTimes!=='undefined'?PrayerTimes:[]).filter(p=>p.name!=='الشروق'&&/^\d\d:\d\d$/.test(p.time)).forEach(p=>{const[h,m]=p.time.split(':').map(Number);const at=new Date();at.setHours(h,m,0,0);const diff=Math.floor((at-now)/60000);const key=`zad_alert_${at.toDateString()}_${p.name}_${s.minutes}`;
      if(diff>=0&&diff<=s.minutes&&!sessionStorage.getItem(key)){const remaining=diff<1?'أقل من دقيقة':`${diff} ${diff===1?'دقيقة':'دقائق'}`;new Notification(`اقتربت صلاة ${p.name}`,{body:`باقي نحو ${remaining} — زاد الذاكرين`,icon:'icons/icon-192.png',tag:key});sessionStorage.setItem(key,'1')}});
  }
