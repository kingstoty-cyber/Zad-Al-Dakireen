...
let activeDhikrCategory = Storage.load('adhkar_active_category') || 'daily';
...
function ensureDailyAdhkarProgress() {
    ...
    [...(AdhkarDB.daily || [])].forEach(item => delete progress[item.id]);
...
function renderAdhkar(category = 'daily') {
    if (!AdhkarDB[category]) category = 'daily';
    ...
    const categories = {
        daily: { name: 'أذكار اليوم والليلة', icon: 'fa-sun' },
        hisn: { name: 'أبواب حصن المسلم', icon: 'fa-book-open' }
    };
            ...
            <div class="card dhikr-card" id="adhkar-${item.id}">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 10px;">
                    <div class="card-title" style="margin: 0;">
                        <i class="fas fa-quote-right"></i> ${item.section || item.reference}
                    </div>
                ...
                <div style="margin-top: 10px; font-size: .82rem; color: var(--accent-color);">${item.reference}</div>
                ${item.benefit ? `<div style="margin-top: 8px; padding: 8px 10px; border-right: 3px solid var(--primary-color); background: rgba(255,255,255,.04); font-size: .9rem;">الفائدة الواردة: ${item.benefit}</div>` : ''}
...
function resetDhikrCategory(category) {
    if (!confirm(`هل تريد تصفير ${category === 'daily' ? 'أذكار اليوم والليلة' : 'هذا القسم'}؟`)) return;
    const progress = Storage.load('adhkar_progress') || {};
...
function renderSettings() {
    ...
    content.innerHTML = `
        ...
        <div class="card">
            ...
            <p>تطبيق <span style="color: var(--primary-color)">زاد الذاكرين</span> - الإصدار ${window.ZAD_APP?.version || '1.0.0'}</p>
...
async function clearAllData() {
    if (confirm("هل أنت متأكد من حذف بيانات زاد الذاكرين؟ لا يمكن التراجع عن هذا الإجراء!")) {
...
window.onload = async () => {
    // ينتظر بيانات حصن المسلم قبل إظهار واجهة التطبيق أو الأذكار.
    if (window.VerifiedAdhkarReady) await window.VerifiedAdhkarReady;
